...
class MainWindow(QMainWindow):
    ...
    def connect_lockin(self):
        ...
        if self.settings.lockin_model == 'SR844':
            ...
            if did_comms_fail is True:
                ...
                # self.zi_lockin.load_settings(r'C:\Users\padmr\Desktop\QIS\PADMR\padmr\supp\settings\2021-06-16_Defaults_UHFLI.xml')
                ...
                # self.zi_lockin.update_all()
                self.zi_lockin.load_settings(r'C:\Users\padmr\Desktop\QIS\PADMR\padmr\supp\settings\2021-06-16_Defaults_UHFLI.xml')
                self.zi_lockin.get_current_settings(self.settings.ui.uhfli_demodulator_idx_spbx.value())
                # self.zi_lockin_delay = self.settings.settling_delay_factor * self.settings.ui.uhfli_time_constant_spbx.value()
    ...
    def get_lockin_results(self, demod=None):
        ...
        if self.settings.lockin_model == 'SR830' or self.settings.lockin_model == 'SR844':
            ...
            if self.internal_reference or self.abscissae[0] == 6 or self.abscissae[1] == 6:
        ...
        if not self.is_averaging_pts and not self.is_recording_transient:  # Single snapshot measurement at each point
            if self.settings.lockin_model == 'SR844' or self.settings.lockin_model == 'SR830':
                ...
                sample = self.zi_lockin.collect_sample(demodulator=demod)
    ...
    def save_file_dialog(self):
        ...
        file_name, file_type = QFileDialog.getSaveFileName(self, "Save File As:", r"C:\Users\padmr\Desktop\Data",
    ...
    def set_icons(self):
        ...
        # refresh_icon.addPixmap(QtGui.QPixmap(r"C:\Users\padmr\Desktop\QIS\PADMR\padmr\supp\icons"
        #                                      r"refresh_icon.png"), QtGui.QIcon.Normal, QtGui.QIcon.Off)
        ...
        # self.settings.ui.uhfli_get_current_settings_btn.setIcon(refresh_icon)
    ...
    def connect_signals_and_slots(self):
        ...
        self.settings.ui.uhfli_range_spbx.editingFinished.connect(
            lambda: self.zi_lockin.set_range(input_idx=self.settings.ui.uhfli_input_cbx.currentIndex(),
                                             input_range=self.settings.ui.uhfli_range_spbx.value()))
        self.settings.ui.uhfli_range_spbx_2.editingFinished.connect(
            lambda: self.zi_lockin.set_range(input_idx=self.settings.ui.uhfli_input_cbx_2.currentIndex(),
                                             input_range=self.settings.ui.uhfli_range_spbx_2.value()))
        ...
        # self.settings.ui.uhfli_freq_spbx.valueChanged[object].connect(
        #     lambda i: self.zi_lockin.set_ref_freq(demod_idx=self.settings.ui.uhfli_demodulator_idx_spbx.value(), target_freq=i)
        # )

        self.settings.ui.uhfli_freq_spbx.editingFinished.connect(
        ...
        self.settings.ui.uhfli_time_constant_spbx.editingFinished.connect(
            lambda: self.zi_lockin.set_time_constant(tf_primary=True,
                                                       demod_idx=self.settings.ui.uhfli_demodulator_idx_spbx.value(),
                                                       target_tc=self.settings.ui.uhfli_time_constant_spbx.value())
        )
        ...
        self.settings.ui.uhfli_time_constant_spbx_2.valueChanged.connect(
            lambda: self.zi_lockin.set_time_constant(tf_primary=False,
                                                       demod_idx=self.settings.ui.uhfli_demodulator_idx_spbx_2.value(),
                                                       target_tc=self.settings.ui.uhfli_time_constant_spbx_2.value())
        )

#TODO:
# Critical:
# -21. Replace silly signal/slots telephone game by connecting in the connect function
# -20. Should settings be a separate class or should each module have its own settings class? Or should each instrument
# have a class within the settings_main?
# -19. Should the error emit signal be a part of the __init__ function to save space?
# -18. Should the error cluster also contain a type (warning vs error) that determines level of reaction?
# -17. finish up with all the update lockin functions
...
# -13. Connect the "clear Errors" button to functionality
...
# -8. Connect lock-in delay scaling factor spinner to functionality
...
sys.path.extend(['C:\\Users\\Ryan\\Google Drive\\1b. CUBoulder\\PycharmProjects\\QIS', 'C:/Users/Ryan/Google Drive/1b. CUBoulder/PycharmProjects/QIS'])
...
from Lockin_Amplifier.lockin_control_module import PrologixAdaptedSRLockin, ErrorCluster
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.settings.ui.cg635_run_btn.clicked.connect(self.run_btn_test)
    ...
    def set_lockin_settings(self):
        ...
        self.lockin_update_outputs(self.settings.lia.outputs)
        print('setting sensitivity and filter slope')
        self.lockin.write_string('SENS %d; OFSL%d\n' % (self.settings.lia.sensitivity,
                                                        self.settings.lia.filter_slope), read=False)
        print('setting the time constant')
        self.lockin.write_string('OFLT %d\n' % self.settings.lia.time_constant, read=False)
        self.settings.lia.time_constant_value = self.settings.lia.tc_numeric_list[self.settings.lia.time_constant]
        self.lockin_delay = self.settings.lia.time_constant_value * self.settings.lia.settling_delay_factor

        print('setting the sampling rate')
        self.lockin.write_string('SRAT %d\n' % self.settings.lia.sampling_rate, read=False)
        self.lockin.write_string('FMOD %d\n' % self.settings.lia.reference_source, read=False)

        if self.settings.lia.model == 'SR830' and self.lockin.connected is True:
            print('SR830 Is Selected')

            print('seting dynamic reserve')
            self.lockin.write_string('RMOD %d\n' % self.settings.lia.dynamic_reserve, read=False)
            ...
            self.lockin.write_string('WRSV %d; CRSV %d\n' % (self.settings.lia.wide_reserve,
                                                             self.settings.lia.close_reserve), read=False)
            print('setting impedances')
            self.lockin.write_string('INPZ %d; REFZ %d\n' % (self.settings.lia.input_impedance,
                                                             self.settings.lia.reference_impedance), read=False)
            print('Setting 2F mode')
            self.lockin.write_string('HARM %d\n' % self.settings.lia.twoF_detect_mode, read=False)

            print('setting Expand')
            if self.settings.lockin_outputs == 0:
                self.lockin.write_string('DEXP 1,1,%d\n' % self.settings.lia.expand, read=False)
                self.lockin.write_string('DEXP 2,1,%d\n' % self.settings.lia.expand, read=False)
            elif self.settings.lockin_outputs == 1:
                self.lockin.write_string('DEXP 1,0,%d\n' % self.settings.lia.expand, read=False)
                # self.lockin.write_string('DEXP 2,0,%d\n' % self.settings.lia.expand, read=False)
            else:
                print('Only X/Y and R/Theta Are set up for expand')
    ...
    def data_collection_tasks(self, filename=None, filetype=None):
        ...
        while jj < num_scans:
            ...
            if self.abort_scan is True:
                break
            elif self.pause_scan is True:
                time.sleep(0.05)
                pause_timer_jj = pause_timer_jj + 0.05
                if pause_timer_jj > 120:
                ...
                while ii < num_steps:
                    ...
                    if self.abort_scan is True:
                        ...
                        time.sleep(0.05)
                        pause_timer_ii = pause_timer_ii + 0.05
                        if pause_timer_ii > 120:
                            self.abort_scan = True
                    else:
                        ...
                        if self.abscissa_1 is 0:  # Empty Header line
                            ...
                            if self.cg635_connected is False:
                                ...
                                if self.cg635.error_dict['Status']:
    ...
    # @QtCore.pyqtSlot(object)
    # def recieve_error_signal(self, error_cluster):
    #     pass

    # ------------------------------------ UPDATE LOCKIN STUFF BUTTONS ----------------------------
    # @QtCore.pyqtSlot()
    # def lockin_auto_crsrv(self):
    #     self.lockin.write_string('ACRS\n', read=False)
    #
    # @QtCore.pyqtSlot()
    # def lockin_auto_dyn_rsrv(self):
    #     self.lockin.write_string('ARSV\n', read=False)
    #
    # @QtCore.pyqtSlot()
    # def lockin_auto_wrsrv(self):
    #     self.lockin.write_string('AWRS\n', read=False)
    #     self.settings.lia.wide_reserve = int(self.lockin.write_string('WRSV?\n', read=True))
    #
    # @QtCore.pyqtSlot()
    # def lockin_auto_offset(self):
    #     """Offsets R"""
    #     if self.settings.lockin_model_preference == 'SR844':
    #         self.lockin.write_string('AOFF 1, 1\n', read=False)
    #     elif self.settings.lockin_model_preference == 'SR830':
    #         self.lockin.write_string('AOFF 3\n', read=False)
    #
    # @QtCore.pyqtSlot()
    # def lockin_auto_phase(self):
    #     self.lockin.write_string('APHS\n', read=False)
    #     self.settings.lia.phase = self.lockin.write_string('PHAS?\n', read=True)

    # @QtCore.pyqtSlot(int)
    # def lockin_update_crsrv(self, idx):
    #     self.lockin.write_string('CRSV %d\n' % idx, read=False)
    #     if not self.lockin.error.status:
    #         self.settings.lia.close_reserve = idx
    #
    # @QtCore.pyqtSlot(int)
    # def lockin_update_dyn_rsrv(self, idx):
    #     self.lockin.write_string('RMOD %d\n' % idx, read=False)
    #     if not self.lockin.error.status:
    #         self.settings.lia.dynamic_reserve = idx
    #
    # @QtCore.pyqtSlot(int)
    # def lockin_update_expand(self, idx):
    #     if self.settings.lockin_model_preference == 'SR844' and self.settings.lia.outputs == 0:
    #         self.lockin.write_string('DEXP 1,1,%d\n' % idx, read=False)
    #         self.lockin.write_string('DEXP 2,1,%d\n' % idx, read=False)
    #     elif self.settings.lockin_model_preference == 'SR844' and self.settings.lia.outputs == 1:
    #         self.lockin.write_string('DEXP 1,0,%d\n' % idx, read=False)
    #         # self.lockin.write_string('DEXP 2,0,%d\n' % self.settings.sr844_expand, read=False)
    #     elif self.settings.lockin_model_preference == 'SR830' and self.settings.lia.outputs == 0:
    #         self.lockin.write_string('OEXP 3,0,%d\n' % idx, read=False)
    #     elif self.settings.lockin_model_preference == 'SR830' and self.settings.lia.outputs == 1:
    #         self.lockin.write_string('OEXP 1,0,%d\n' % idx, read=False)
    #         self.lockin.write_string('OEXP 2,0,%d\n' % idx, read=False)
    #
    #     if not self.lockin.error.status:
    #         self.settings.lia.expand = idx
    #
    # @QtCore.pyqtSlot(int)
    # def lockin_update_filter_slope(self, idx):
    #     self.lockin.write_string('OFSL%d\n' % idx, read=False)
    #     if not self.lockin.error.status:
    #         self.settings.lia.filter_slope = idx

    @QtCore.pyqtSlot(str, int)
    def update_lockin_property(self, property_name, new_value):
        """
        When a settings window object is activated, a signal emits which is directed straight to the lockin control
        module, telling it to perform some action. Once the module function is complete, a signal is emitted from that
        module, directed to this slot. The purpose of this is to keep the lockin.settings and settings.lockin constantly
    ...
    # @QtCore.pyqtSlot(int)
    # def lockin_update_2f(self, idx):
    #     self.lockin.write_string('HARM %d\n' % idx, read=False)
    #     if not self.lockin.error.status:
    #         self.settings.lia.twoF_mode = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_harmonic(self, value):
        self.lockin.write_string('HARM %d\n' % value, read=False)
        if not self.lockin.error.status:
            self.settings.lia.harmonic = value

    @QtCore.pyqtSlot()
    def lockin_update_phase(self):
        phase_to_set = self.settings.ui.phase_spinner.value()
        self.lockin.write_string('PHAS %d\n' % phase_to_set, read=False)
        if not self.lockin.error.status:
            self.settings.lia.phase = phase_to_set

    @QtCore.pyqtSlot(int)
    def lockin_update_input_impedance(self, idx):
        self.lockin.write_string('INPZ %d\n' % idx, read=False)
        if not self.lockin.error.status:
            self.settings.lia.input_impedance = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_outputs(self, idx):
        if self.settings.lia.model == 'SR844' and idx == 0 and self.lockin.connected is True:
            print('attempting to set outputs to R and Theta')
            self.lockin.write_string('DDEF 1, 1\n', read=False)
            self.lockin.write_string('DDEF 2, 1\n', read=False)
        elif self.settings.lia.model == 'SR844' and idx == 1:
            self.lockin.write_string('DDEF 1, 0\n', read=False)
            self.lockin.write_string('DDEF 2, 0\n', read=False)
        elif self.settings.lia.model == 'SR830' and idx == 0:
            self.lockin.write_string('DDEF 1, 1, 0\n', read=False)
            self.lockin.write_string('DDEF 2, 1, 0\n', read=False)
        elif self.settings.lia.model == 'SR830' and idx == 1:
            self.lockin.write_string('DDEF 1, 0, 0\n', read=False)
            self.lockin.write_string('DDEF 2, 0, 0\n', read=False)

        if not self.lockin.error.status:
            self.settings.lia.outputs = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_ref_impedance(self, idx):
        self.lockin.write_string('REFZ %d\n' % idx, read=False)
        if not self.lockin.error.status:
            self.settings.lia.reference_impedance = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_ref_source(self, idx):
        self.lockin.write_string('FMOD %d\n' % idx, read=False)
        if not self.lockin.error.status:
            self.settings.lia.reference_source = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_sampling_rate(self, idx):
        self.lockin.write_string('SRAT %d\n' % idx, read=False)
        if not self.lockin.error.status:
            self.settings.lia.sampling_rate = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_sensitivity(self, idx):
        self.lockin.write_string('SENS %d\n' % idx, read=False)
        if not self.lockin.error.status:
            self.settings.lia.sensitivity = idx

    @QtCore.pyqtSlot(int)
    def lockin_update_time_constant(self, idx):
        self.lockin.write_string('OFLT %d\n' % idx, read=False)
        if not self.lockin.error.status:
            self.settings.lia.time_constant = idx
            self.settings.lia.time_constant_value = self.settings.lia.tc_numeric_list[idx]
    ...
    def connect_signals_and_slots(self):
        ...
        self.lockin.property_updated_signal.connect(self.settings.lockin_property_updated)

        # self.settings.lockin_2f_signal.connect(self.lockin_update_2f)
        # self.settings.lockin_auto_crsrv_signal.connect(self.lockin_auto_crsrv)
        # self.settings.lockin_auto_dyn_rsrv_signal.connect(self.lockin_auto_dyn_rsrv)
        # self.settings.lockin_auto_wrsrv_signal.connect(self.lockin_auto_wrsrv)
        # self.settings.lockin_auto_offset_signal.connect(self.lockin_auto_offset)
        # self.settings.lockin_auto_phase_signal.connect(self.lockin_auto_phase)
        # self.settings.lockin_close_reserve_signal.connect(self.lockin_update_crsrv)
        # self.settings.lockin_dynamic_reserve_signal.connect(self.lockin_update_dyn_rsrv)        #
        # self.settings.lockin_expand_signal.connect(self.lockin_update_expand)
        # self.settings.lockin_filter_slope_signal.connect(self.lockin_update_filter_slope)
        # self.settings.lockin_harmonic_signal.connect(self.lockin_update_harmonic)
        # self.settings.lockin_phase_signal.connect(self.lockin_update_phase)
        # self.settings.lockin_input_impedance_signal.connect(self.lockin_update_input_impedance)
        # self.settings.lockin_outputs_signal.connect(self.lockin_update_outputs)
        # self.settings.lockin_ref_impedance_signal.connect(self.lockin_update_ref_impedance)
        # self.settings.lockin_ref_source_signal.connect(self.lockin_update_ref_source)
        # self.settings.lockin_sampling_rate_signal.connect(self.lockin_update_sampling_rate)
        # self.settings.lockin_sensitivity_signal.connect(self.lockin_update_sensitivity)
        # self.settings.lockin_time_constant_signal.connect(self.lockin_update_time_constant)
        ...
        self.settings.ui.phase_spinner.valueChanged['int'].connect(self.lockin.update_phase)

...
class MainWindow(QMainWindow):
    ...
    def data_collection_tasks(self, filename=None, filetype=None):
        ...
        if num_dims == 1:
            ch1_curscan_df = pd.DataFrame(columns=[self.column_headers[0]] + (np.arange(1, num_scans + 1)).tolist())
            ch2_curscan_df = pd.DataFrame(columns=[self.column_headers[0]] + (np.arange(1, num_scans + 1)).tolist())
            ave_data_df = pd.DataFrame(columns=self.column_headers)
        elif num_dims == 2:
            ch1_curscan_df = pd.DataFrame(columns=[self.column_headers[0]] + (np.arange(1, num_scans + 1)).tolist())
            ch2_curscan_df = pd.DataFrame(columns=[self.column_headers[0]] + (np.arange(1, num_scans + 1)).tolist())
            ave_data_df = pd.DataFrame(columns=['X/Y'] + self.step_pts[1].tolist())

        print('DataFrames:')
        print(ch1_curscan_df)
        print(ch2_curscan_df)
        print(ave_data_df)

        ch_1_ave = np.zeros((num_scans, self.step_count[0]))
        ch_2_ave = np.zeros((num_scans, self.step_count[0]))

        ch_1_array_average = np.zeros((self.step_count[0], 1))
        ch_2_array_average = np.zeros((self.step_count[0], 1))

        stored_ave_data_matrix = np.zeros((self.step_count[0], 3))
        current_scan_data_matrix = np.zeros((self.step_count[0], 3))
        current_scan_col_headers = [self.column_headers[0], 'Channel 1', 'Channel 2']

        actual_x_values = np.zeros((self.step_count[0], 1))
        ...
        while mm < num_scans_dim2:
            ...
            while ll < self.step_count[1]:
                ...
                if num_dims == 2:
                    print('Next Step (dim2) is: ' + str(next_step) + '(' + str(ll + 1) + ' of ' + str(self.step_count[1]) + ')')
                    next_step_dim2 = self.step_pts[1][ll]
                    self.set_abscissa(which_abscissa=1, next_step=next_step_dim2)
                ...
                while jj < num_scans:
                    ...
                    while ii < self.step_count[0]:
                        ...
                        current_position = self.set_abscissa(which_abscissa=0, next_step=next_step)
                        if jj == 0:
                            actual_x_values[ii] = current_position
                            ch1_curscan_df.iloc[ii, 0] = current_position
                        ...
                        # HERE A DF CHANGE WAS MADE
                        x, y = self.get_lockin_results(duration)
                        ch1_curscan_df.iloc[ii, jj], ch2_curscan_df.iloc[ii, jj] = x, y
                        ch_1_ave[jj, ii], ch_2_ave[jj, ii] = self.get_lockin_results(duration)
                        ...
                        # HERE A DF CHANGE WAS MADE
                        ch1_df_mean = ch1_curscan_df.mean(axis=1)
                        ch2_df_mean = ch2_curscan_df.mean(axis=1)
                        ave_data_df.insert(0, column=self.column_headers[0], value=actual_x_values)
                        ave_data_df.insert(1, column='ch1', value=ch1_df_mean)
                        ave_data_df.insert(2, column='ch2', value=ch2_df_mean)
                        print('Ave_data_df:')
                        print(ave_data_df)
                        ...
                        line1, line2, line3, line4 = self.plot_results_df(ii, jj, actual_x_values,
                                                                       ch1_curscan_df, ch2_curscan_df, ave_data_df,
                                                                       line1, line2, line3, line4)

                        # line1, line2, line3, line4 = self.plot_results(ii, jj, actual_x_values,
                        #                                                ch_1_array_current, ch_2_array_current,
                        #                                                ch_1_array_average, ch_2_array_average,
                        #                                                line1, line2, line3, line4)
                    ...
                    if filename is None:
                        ...
                        current_scan_data_matrix[:, 0] = actual_x_values.flatten()
        ...
        stored_ave_data_matrix[:, 0] = actual_x_values.flatten()
    ...
    def plot_results(self, ii, jj, actual_x_values, ch_1_array_current, ch_2_array_current,
                     ch_1_array_average, ch_2_array_average, line1, line2, line3, line4):
